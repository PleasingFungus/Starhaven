package org.flixel
{
	...
	public class FlxTilemap extends FlxObject
	{
		...
		public function loadMap(MapData:String, TileGraphic:Class, TileWidth:uint=0, TileHeight:uint=0):FlxTilemap
		{
			...
			{
				cols = rows[r].split(",");
				if (cols.length <= 1 && rows[r].indexOf(';') != -1) { //NOTE: This line is modified from vanilla flixel!
					heightInTiles--;
					continue;
				}
				if(widthInTiles == 0)
		 ...
		 *
		 * @param	X		The X coordinate of the tile (in pixels, not tiles).
		 * @param	Y		The Y coordinate of the tile (in pixels, not tiles).
		 *
		 * @return	A uint containing the value of the tile at this spot on the screen. (Not adjusted for scroll.)
		 */
		public function getTileByPixels(X:uint,Y:uint):uint
		{
			return getTileByIndex(int(Y/_tileHeight) * widthInTiles + int(X/_tileWidth));
		}
		 ...
		 */
		public function setTileByPixels(X:uint, Y:uint, Tile:uint, UpdateGraphic:Boolean = true):Boolean
		{
			return setTileByIndex(int(Y/_tileHeight) * widthInTiles + int(X/_tileWidth), Tile, UpdateGraphic);
		}
